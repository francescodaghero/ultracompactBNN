...
void __attribute__((always_inline)) xnorpop4x1_linear(
        ...
        const int32_t *thresholds)
{
    ...
    uint32_t A4, A4_next;
    uint32_t *pA4_next, *pA4;


    int feature_out;
    int output_features_leftover= dim_out%4;
    ...
    uint32_t weights_mask2, weights_mask3, weights_mask4;
    int thr_feature_index = 0;

    //Iterating over output features/classes
    for(feature_out = 0; feature_out< dim_out>>2; feature_out+=1) {
        ...
        //Successive sets of weights
        uint32_t weights_overstep4 = MODULO_32(weights_overstep3+words_infeatures_leftover);
        pA4 = pA3 + words_infeatures + ((weights_overstep3+words_infeatures_leftover) >=32); //Point pA2 all'inizio dei pesi
        int next_weights_in_next_word4 = ((weights_overstep4 + words_infeatures_leftover) >=32) | next_pA;
        ...
        for (int words_in = 0; words_in < words_infeatures; words_in++) {
            ...
            uint32_t incr4 = ((words_infeatures_leftover + weights_overstep4) >= 32) | ( words_in != (words_infeatures - 1));
            pA4_next += incr4;
            A4_next = (*(pA4_next));
            A4 = A4 | ( (A4_next >> (32-weights_overstep4)) & weights_mask4);
        ...
        accum4 += POPCOUNT(XNOR(B,A4) & leftover_mask);
        ...
        *(pOut++) =(int32_t) (accum4 * thresholds[thr_feature_index++] + thresholds[thr_feature_index++]);

        weights_overstep1 = MODULO_32((weights_overstep4 + dim_feat));
