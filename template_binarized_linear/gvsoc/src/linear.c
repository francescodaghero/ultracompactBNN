...
void __attribute__((always_inline)) linear(
...
)
{
    ...
    uint32_t leftover_mask =~((1 << (32 - words_infeatures_leftover )) - 1); //TODO Check with an input multiple of 32
    ...
    for(int feature_out = 0; feature_out< dim_out; feature_out++) {
        ...
        *(pOut++) =(int32_t) (accum * thresholds[feature_out<<2] + thresholds[(feature_out<<2)+1]); //Perform batch norm
        ...
        A = A2;
        A <<= weights_overstep;
        pA2 += ((weights_overstep + words_infeatures_leftover) >=32) | (dim_feat>=32);
        A2 = *(pA2);
