...
uint32_t *xnorpop4x4(
    ...
    for(int out_channel=0; out_channel< (ch_out>>2); out_channel++) {
        ...
        for (int words_in = 0; words_in < words_kernel; words_in++) {
            ...
            accum2_ts2 += POPCOUNT(XNOR(B3,A2));
            accum3_ts2 += POPCOUNT(XNOR(B3,A3));
            accum4_ts2 += POPCOUNT(XNOR(B3,A4));

            accum1_ts3 += POPCOUNT(XNOR(B4,A1));
            accum2_ts3 += POPCOUNT(XNOR(B4,A2));
            accum3_ts3 += POPCOUNT(XNOR(B4,A3));
            accum4_ts3 += POPCOUNT(XNOR(B4,A4));
        ...
        accum2_ts2 += POPCOUNT(XNOR(B3,A2) & leftover_mask);
        accum3_ts2 += POPCOUNT(XNOR(B3,A3) & leftover_mask);
        accum4_ts2 += POPCOUNT(XNOR(B3,A4) & leftover_mask);

        accum1_ts3 += POPCOUNT(XNOR(B4,A1) & leftover_mask);
        accum2_ts3 += POPCOUNT(XNOR(B4,A2) & leftover_mask);
        accum3_ts3 += POPCOUNT(XNOR(B4,A3) & leftover_mask);
        accum4_ts3 += POPCOUNT(XNOR(B4,A4) & leftover_mask);
        ...
        if(accum2_ts0 >= *(thresholds)) {
            z |= out_mask;
        }
        if(accum2_ts1 >= *(thresholds)) {
            z2 |= out_mask;
        }
        if(accum2_ts2 >= *(thresholds)) {
            z3 |= out_mask;
        }
        if(accum2_ts3 >= *(thresholds++)) {
            z4 |= out_mask;
        }
        ...
        if(accum3_ts0 >= *(thresholds)) {
            z |= out_mask;
        }
        if(accum3_ts1 >= *(thresholds)) {
            z2 |= out_mask;
        }
        if(accum3_ts2 >= *(thresholds)) {
            z3 |= out_mask;
        }
        if(accum3_ts3 >= *(thresholds++)) {
            z4 |= out_mask;
        }
        ...
        if(accum4_ts0 >= *(thresholds)) {
            z |= out_mask;
        }
        if(accum4_ts1 >= *(thresholds)) {
            z2 |= out_mask;
        }
        if(accum4_ts2 >= *(thresholds)) {
            z3 |= out_mask;
        }
        if(accum4_ts3 >= *(thresholds++)) {
            z4 |= out_mask;
        }
