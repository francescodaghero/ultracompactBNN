...
//DECLARE VARIABLES
    ...
    for(int out_channel=0; out_channel< (ch_out)>>2; out_channel++) {
        ...
        tmp2 = (tmp2 << 1) + (accum3_ts2 >= *(thresholds));
        tmp3 = (tmp3 << 1) + (accum3_ts3 >= *(thresholds));
        tmp4 = (tmp4 << 1) + (accum3_ts4 >= *(thresholds++));
        tmp1 = (tmp1 << 1) + (accum4_ts1 >= *(thresholds));
        tmp2 = (tmp2 << 1) + (accum4_ts2 >= *(thresholds));
        tmp3 = (tmp3 << 1) + (accum4_ts3 >= *(thresholds));
        tmp4 = (tmp4 << 1) + (accum4_ts4 >= *(thresholds++));

        z1 |= tmp1 << (32 - savecounter1 - 4);
        z2 |= tmp2 << (32 - savecounter2 - 4);
        z3 |= tmp3 << (32 - savecounter3 - 4);
        z4 |= tmp4 << (32 - savecounter4 - 4);
